...
import { FaUtensils, FaArrowRight, FaStar, FaClock, FaCheck, FaPlay, FaUsers, FaHeart, FaBolt, FaSeedling, FaWallet } from "react-icons/fa";
...
export default function Home() {
  ...
  const handleCategoryClick = (categoryId, categoryName) => {
    ...
    router.push(`/user/menu?category=${categoryId}`)
  };
        ...
        <section id="categories" className="py-12 sm:py-16 lg:py-20 bg-slate-50">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="text-center mb-8 sm:mb-12 lg:mb-16">
              <div className="inline-flex items-center px-3 py-1 sm:px-4 sm:py-2 rounded-full bg-yellow-100 text-yellow-800 text-xs sm:text-sm font-semibold mb-3 sm:mb-4">
                🍽️ Our Menu
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6 lg:gap-8">
              {categories.map((category, index) => (
                <div
                  ...
                  className="group cursor-pointer bg-white rounded-2xl sm:rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:scale-105 border border-slate-200 overflow-hidden"
                ...
                >
                  ...
                  <div className="relative h-40 sm:h-48 overflow-hidden">
                    {category.imgUrl ? (
                      ...
                      <div className="w-full h-full bg-gradient-to-br from-yellow-400 to-yellow-600 flex items-center justify-center">
                        <FaUtensils className="h-12 w-12 sm:h-16 sm:w-16 text-white opacity-80" />
                      </div>
                    )}
                    {/* Gradient Overlay */}
                    ...
                    {/* Hover Arrow */}
                    <div className="absolute top-2 right-2 sm:top-4 sm:right-4 transform translate-x-2 opacity-0 group-hover:translate-x-0 group-hover:opacity-100 transition-all duration-300">
                      <div className="bg-white text-slate-900 p-1.5 sm:p-2 rounded-full shadow-lg">
                  ...
                  <div className="p-4 sm:p-6">
                    <h3 className="text-lg sm:text-xl font-bold text-slate-900 mb-1 sm:mb-2 text-center group-hover:text-yellow-600 transition-colors duration-300">
                      {category.categoryName}
                    </h3>
                    <p className="text-slate-600 text-xs sm:text-sm text-center">
                      Explore {category.categoryName.toLowerCase()} items
                    </p>
                  </div>
                </div>
              ))}
            ...
            <div className="text-center mt-8 sm:mt-12">
              <button className="inline-flex items-center px-6 py-3 sm:px-8 sm:py-4 bg-slate-900 text-white rounded-2xl font-semibold hover:bg-slate-800 transition-all duration-300 transform hover:scale-105 shadow-lg group text-sm sm:text-base">
                <span>View Complete Menu</span>
                <FaArrowRight className="ml-2 sm:ml-3 h-3 w-3 sm:h-4 sm:w-4 group-hover:translate-x-1 transition-transform" />
              </button>
            </div>
          </div>
        </section>
